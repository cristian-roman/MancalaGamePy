...
class BoardComponent(GameComponent):
    ...
    def __init__(self, window: pygame.Surface, images_path):
        ...
        self.board = pygame.transform.scale(self.board, (AppSettings.board_width, AppSettings.board_height))
        board_x = self.window.get_width() // 2 - AppSettings.board_width // 2
        board_y = self.window.get_height() // 2 - AppSettings.board_height // 2 + 50
        ...
        self.right_pot = PotComponent(window,
                                      (self.board_coordinates[0] + AppSettings.board_width - 100,
                                       self.board_coordinates[1] + AppSettings.board_height - PotComponent.pot_height + 100),
                                      images_path)

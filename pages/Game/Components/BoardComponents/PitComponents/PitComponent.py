...
class PitComponent(GameComponent):
    ...
    Class-level attributes:
        ...
        DEFAULT_STONE_NUMBER (int): The default number of stones contained in the pit.
        DEFAULT_HIGHLIGHT_WIDTH (int): The default width of the ellipse that highlights the pit.
        DEFAULT_LABEL_SIZE (int): The default size of the label that shows the number of stones contained in the pit.

    Attributes:
        window (pygame.Surface): The window where the pit is drawn.
        pit_coordinates (tuple): The coordinates of the pit on the table.
    ...
    def __init__(self, window, pit_coordinates, pit_index):
        """This constructor initializes the attributes of the PitComponent.

        Moreover, it initializes the following attributes:
            is_highlighted (bool):  True if the pit is highlighted, False otherwise.
                                    This attribute is used to control the highlighting of the pit.

            stones (list):  The list of stones contained in the pit generated randomly.

            label (LabelComponent): The label that shows the number of stones contained in the pit with its attributes:
                                    position (tuple): The position of the label relative to pit X coordinate.
                                    text (str): The text of the label. At this step DEFAULT_STONE_NUMBER.
                                    size (int): The size of the label. The value is always DEFAULT_LABEL_SIZE.
                                    color (tuple): The color of the text from label. It is always white.
                                    has_background (bool): It is always True.
                                    background_color (tuple):   The color of the background of the label.
                                                                It is picked by querying the owner of the pit.
                                    background_shape (str): The shape of the background of the label. It is always ellipse.
        Args:
            window (pygame.Surface): The window where the pit is drawn.
        ...
        self.label = LabelComponent(self.window,
                                    position=(self.pit_coordinates[0] + self.PIT_WIDTH // 2, label_y),
    ...
    def __draw_ellipse(self):
        """
        Draw the ellipse that highlights the pit in the zone defined by pit coordinates and size
        with the highlight width and the background color defined in constructor
        """
        zone = pygame.Rect(self.pit_coordinates[0], self.pit_coordinates[1],
                           self.PIT_WIDTH, self.PIT_HEIGHT)
        pygame.draw.ellipse(self.window, self.background_color, zone, self.highlight_width)

    def __erase_ellipse(self):
        """Erase the ellipse that highlights the pit by drawing a transparent ellipse in the same zone."""
    ...
    def _draw(self):
        """
        Draw the pit with its stones and the label that shows the number of stones contained in the pit.
        If the pit is highlighted, draw the ellipse that highlights the pit.
    ...
    def is_mouse_hovering(self, mouse_position):
        """
        Check if the mouse is hovering the pit by checking if the mouse position is in the zone defined by
        pit coordinates and size.
        It sets the highlight width to default value for the cases when the mouse is not hovering the pit.
    ...
    def treat_hovering(self):
        """
        Sets the highlight width to 10 and redraw the pit if the pit is not highlighted.
        """
    ...
    def add_stone(self, stone):
        """Add a stone that was picked from another pit to the current pit."""
